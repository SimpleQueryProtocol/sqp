/*
 * Copyright 2015 by Rothmeyer Consulting (http://www.rothmeyer.com/)
 * Author: Stefan Burnicki <stefan.burnicki@burnicki.net>
 *
 * This file is part of SQP.
 *
 * SQP is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License.
 *
 * SQP is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with SQP.  If not, see <http://www.gnu.org/licenses/>.
 */

package io.sqp.core.types;

import io.sqp.core.exceptions.TypeConversionException;
import io.sqp.core.util.TypeUtil;

import java.util.Base64;

/**
 * @author Stefan Burnicki
 * @see SqpBinary
 */
public class SqpVarBinary extends SqpValue {
    private byte[] _value;

    public SqpVarBinary(byte[] value) {
        super(SqpTypeCode.VarBinary);
        _value = value;
    }

    @Override
    public byte[] getJsonFormatValue() {
        return _value;
    }

    @Override
    public byte[] asBytes() throws TypeConversionException {
        return _value;
    }

    public static SqpVarBinary fromJsonFormatValue(Object value) {
        if (value instanceof byte[]) {
            return new SqpVarBinary((byte[]) value);
        }
        String encoded = TypeUtil.checkAndConvert(value, String.class, "Base64 encoded value");
        return new SqpVarBinary(Base64.getDecoder().decode(encoded));
    }
}
